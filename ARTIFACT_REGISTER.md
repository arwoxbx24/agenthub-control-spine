...
| `governance/model-portfolio-utilization-policy.md` | `active_policy` | false | false | AH-615 | Model portfolio route matrix and continuous task-loop policy; Spark primary for code/config and no owner manual model selection. |
| `validators/model_portfolio_router_validator.rb` | `active_control` | false | false | AH-615 | Deterministic validator for model portfolio routes, Spark canary, fallback canary, continuous loop, and Done evidence. |
| `evals/model-portfolio-routing/fixtures.json` | `active_control` | false | false | AH-615 | Positive and negative fixtures for model route classes, sandbox-not-Spark proof, owner model selection denial, fallback, and Done gate. |
