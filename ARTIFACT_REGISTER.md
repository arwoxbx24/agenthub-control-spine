...
| `reports/audits/P0-result-only-spark-route-restore-prompt-audit-20260529.md` | `audit_only` | false | false | AH-590/AH-601 | Read-only audit of result-only Spark route restore prompt: confirms route mostly installed, finds one stale runtime-policy sentence, and blocks stale-folder deletion without exact path binding. |
