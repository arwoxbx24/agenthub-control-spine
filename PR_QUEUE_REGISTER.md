...
|---|---|---|---|---|---|---|---|---|
| `#29` | `AH-526` | `UNKNOWN_NOT_EXPOSED` | `agenthub/ah-526-session-failure-report` | T0 Registrar | `MERGE_READY` | Incident report moved from GitHub Issue surface into repository artifact file, indexed, registered as audit-only, and paired with receipt. Merge only after required checks pass. | `reports/incidents/AH-526-session-failure-report.md`; `reports/receipts/AH-526-session-failure-report-receipt.md`; Issue `#28` readback | none |
...
| PR | Decision | Blocker |
|---|---|---|
| `#29` | AH-526 incident-report PR; merge-ready after required checks pass; report remains audit-only and non-default | none |
